...
const Reports = () => {
    ...
    // Default date range: 1st of current month to end of current month
    ...
    useEffect(() => {
        ...
        // Listen for customers to get names
    ...
    const totalGive = filteredTx.filter(tx => tx.amount < 0).reduce((acc, tx) => acc + Math.abs(tx.amount), 0);
    const totalGot = filteredTx.filter(tx => tx.amount > 0).reduce((acc, tx) => acc + tx.amount, 0);
            ...
            <div className="flex flex-1 ml-0 md:ml-[260px] overflow-hidden">
                ...
                <div className="hidden md:flex w-[280px] bg-white border-r border-gray-200 flex-col flex-shrink-0">
                    ...
                    <div className="px-2">
                        ...
                        <div className="relative group">
                            ...
                            <button className="w-full flex items-center gap-4 px-4 py-4 bg-blue-50 text-blue-600 transition-colors">
                                <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white">
                                    <span className="material-symbols-outlined text-[20px]">swap_horiz</span>
                                </div>
                                <div className="text-left">
                                    <p className="text-sm font-bold leading-tight">Transaction Report</p>
                                    <p className="text-[11px] text-blue-400 mt-0.5">All customers, All Transactions</p>
                                </div>
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-white">
                    ...
                    <div className="flex items-center justify-between px-8 py-4 border-b border-gray-200">
                        ...
                        {globalSettings?.pdfExport !== false && (
                            <div className="flex items-center gap-3">
                                <button className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded text-xs font-bold text-gray-600 hover:bg-gray-50 transition-colors shadow-sm">
                    ...
                    {/* Tabs */}
                    <div className="px-8 border-b border-gray-100 flex items-center gap-10">
                    ...
                    {/* Filters Row */}
                    <div className="px-4 md:px-8 py-5 border-b border-gray-100 grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 items-end">
                        <div className="col-span-1 md:col-span-3">
                            <p className="text-xs font-bold text-gray-500 mb-2">Customer Name</p>
                            <div className="relative">
                                ...
                                <input 
                                    type="text"
                                    placeholder="Search"
                                    className="w-full pl-9 pr-3 py-2.5 bg-white border border-gray-300 rounded-lg text-sm outline-none focus:border-blue-400 transition-all placeholder:text-gray-300"
                                    value={searchQuery}
                        ...
                        <div className="col-span-1 md:col-span-3">
                            <p className="text-xs font-bold text-gray-500 mb-2">Period</p>
                            <div className="relative">
                                <select 
                                    className="w-full pl-3 pr-9 py-2.5 bg-white border border-gray-300 rounded-lg text-sm outline-none appearance-none cursor-pointer text-gray-700 font-medium"
                                    value={period}
                        ...
                        <div className="col-span-1 md:col-span-3">
                            <p className="text-xs font-bold text-gray-500 mb-2">Start</p>
                            <div className="relative">
                                <span className="material-symbols-outlined absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-[18px]">calendar_today</span>
                                <input 
                                    type="date"
                                    className="w-full pl-9 pr-3 py-2.5 bg-white border border-gray-300 rounded-lg text-sm outline-none focus:border-blue-400 transition-all text-gray-700"
                                    value={startDate}
                        ...
                        <div className="col-span-1 md:col-span-3">
                            <p className="text-xs font-bold text-gray-500 mb-2">End</p>
                            <div className="relative">
                                <span className="material-symbols-outlined absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-[18px]">calendar_today</span>
                                <input 
                                    type="date"
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8 pt-6 bg-white custom-scrollbar">
                        <p className="text-sm font-bold text-gray-800 mb-4 px-2 md:px-0">Total {filteredTx.length} entries</p>
                        
                        {/* Summary Area — Responsive Cards */}
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-8 px-2 md:px-0">
                            <div className="bg-[#FCE8E6] p-4 md:p-6 rounded-xl border border-[#F4C7C3] flex flex-col items-center shadow-sm">
                                <span className="text-red-700 text-[10px] md:text-[11px] font-black uppercase tracking-widest mb-1 md:mb-2">TOTAL GAVE</span>
                                <span className="text-red-700 text-xl md:text-2xl font-black">₹{totalGive.toLocaleString('en-IN')}</span>
                            </div>
                            <div className="bg-[#E6F4EA] p-4 md:p-6 rounded-xl border border-[#B7E1CD] flex flex-col items-center shadow-sm">
                                <span className="text-green-700 text-[10px] md:text-[11px] font-black uppercase tracking-widest mb-1 md:mb-2">TOTAL GOT</span>
                                <span className="text-green-700 text-xl md:text-2xl font-black">₹{totalGot.toLocaleString('en-IN')}</span>
                            </div>
                            <div className="hidden md:flex bg-gray-50 p-6 rounded-xl border border-gray-200 flex-col items-center shadow-sm">
                                <span className="text-gray-500 text-[11px] font-black uppercase tracking-widest mb-2">NET BALANCE</span>
                                <span className={`text-2xl font-black ${netBalance >= 0 ? 'text-red-500' : 'text-green-600'}`}>
                                ...
                                return (
                                    <div key={tx.id || idx} className="bg-white border border-gray-100 rounded-xl p-4 flex items-center justify-between gap-4 shadow-sm active:bg-gray-50 transition-colors">
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-600 font-bold flex-shrink-0 border border-gray-100">
